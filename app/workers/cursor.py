"""Cursor CLI controlled subprocess worker."""
...
class CursorWorker:
    ...
    def _command(self) -> str | None:
        cmd = self.config.cursor.command
    ...
    async def run(self, decision: RouteDecision) -> WorkerResult:
        ...
        if binary is None:
            ...
            return WorkerResult(success=False, evidence=evidence, summary="Cursor CLI missing")
        ...
        if not decision.working_directory:
            ...
            return WorkerResult(
                success=False,
                evidence=evidence,
                summary="Select a project before using Cursor CLI.",
            )

        cwd = Path(decision.working_directory)
        before = await collect_git_evidence(cwd)

        cmd = [binary, "-p", decision.instruction]
        try:
            ...
            stdout_b, stderr_b = await asyncio.wait_for(
                proc.communicate(),
                timeout=self.config.cursor.timeout_seconds,
            )
            ...
            evidence = WorkerEvidence(
                worker="cursor",
                action=decision.action,
                stderr="Cursor CLI timed out",
                exit_code=124,
            )
            return WorkerResult(success=False, evidence=evidence, summary="Cursor timed out")
        except Exception as exc:  # noqa: BLE001
            logger.exception("Cursor worker failed")
            evidence = WorkerEvidence(
                worker="cursor",
                action=decision.action,
                stderr=str(exc),
        ...
        summary = stdout.strip() or stderr.strip() or f"Cursor exited {exit_code}"
        return WorkerResult(
            success=exit_code == 0,
            evidence=evidence,
